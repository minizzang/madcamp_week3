...
const AppRouter = ({ isLoggedIn }) => {
  ...
  console.log(
    location.pathname.toString().split("/")[3] === "write"
      ? location.pathname
      : null
  );
    ...
    <TransitionGroup>
      ...
      >
        <Routes location={location}>
          <Route path="mypage/:id" element={<Home />} />{" "}
          {/*  지금은 기본 path인데,  나중에 username으로 각 user 페이지에 접근 가능하도록 해야 할 듯. */}
          ...
          <Route path="/welcome" element={<Welcome />} />
          <Route path="/mypage/:id/write" element={<Write />} />
          {/* 익명 작성이 가능하다면 Id/write 형식 or write/id 형식으로 누구에게 쓰는 건지 특정하게 해야 할 듯 */}
          <Route path="/letter" element={<Letter />} />{" "}
          {/* 나중에 로그인 된 상태라면 개인 페이지로 이동 가능하게 만들 수 있을 듯*/}
