...
const Home = () => {
  ...
  const handleChange = (e) => {
    ...
    memoRef.current.style = `width: ${String(
      Number(temp.slice(0, temp.length - 2)) + 10
    )}px`;
  ...
  const getLettersFromDB = async () => {
    ...
    await axios
      .get(BASE_URL + `/letter/getMyInvalidLetters/${id}`)
      .then((response) => {
    ...
    await axios
      .get(BASE_URL + `/letter/getMyValidLetters/${id}`)
      .then((response) => {
        if (response.data == "편지가 없어요") {
          console.log("no letter!");
        } else {
  ...
  useEffect(() => {
    ...
    if (!loading) {
      ...
      for (i = 0; i < letterValidInfo.length; i++) {
        ...
        written_date.innerText = letterValidContents[i].written_date.substr(
          0,
          10
        );
